...
from config import ENV, configs, model
...
app_config = configs[ENV]


def make_celery(_app_config):
    _celery = Celery('analysis',
                     broker=_app_config.CELERY_BROKER_URL,
                     backend=_app_config.CELERY_RESULT_BACKEND
                     )
    # _celery.conf.update(_app_config)
    return _celery


celery = make_celery(app_config)
